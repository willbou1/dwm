...
static void decedge(const Arg *arg);
...
static void incedge(const Arg *arg);
...
static void sigdwmblocks(const Arg *args);
...
unswallow(Client *c)
{
	...
	XMoveResizeWindow(dpy, c->win, c->w, c->y, c->w, c->h);
...
buttonpress(XEvent *e)
{
	...
	uint8_t onBarWin = 0;
	for (Monitor *curr = mons; curr; curr = curr->next)
		if (ev->window == curr->barwin) { onBarWin = 1; break; }
	if (onBarWin) {
		i = x = 0;
		do {
			if (strcmp(tags[i], hiddentag) != 0)
				x += TEXTW(tags[i]);
...
{
	Client *c;
	unsigned int n, i, r, h, mw, tw, my, lty, rty,
		     mx, rtx, ww, wh, wx, wy;
	...
	/* applying gaps around window area */
	ww = m->ww - (m->edgepx * 2);
	wh = m->wh - (m->edgepx * 2);
	wx = m->wx + m->edgepx;
	wy = m->wy + m->edgepx;
	...
	if (n > m->nmaster) {
		...
		if (!m->nmaster) {
			...
			if (n - m->nmaster > 1) {
				/* Two stacks */
				mw = ww * m->mfact - m->edgepx;
				tw = (ww - mw - 2*m->edgepx) / 2;
				mx = wx + tw + m->edgepx;
				rtx = mx + mw + m->edgepx;
			} else {
				/* One stack */
				mw = ww * m->mfact - (m->edgepx / 2);
				tw = ww - mw - m->edgepx;
				mx = wx + tw + m->edgepx;
			}
		}
	} else {
		/* Only master will be used */
		mw = ww;
		mx = wx;
	}
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			/* Put on stack */
			r = MIN(n, m->nmaster) - i;
			h = (wh - my - ((r - 1) * m->gappx)) / r;
			resize(c, mx, wy + my, mw - (2*c->bw), h - (2*c->bw), False);
			my += HEIGHT(c) + m->gappx;
		} else {
			if ((i - m->nmaster) % 2 == 0) {
				/* Put on left stack */
				r = (1 + n - i) / 2;	
				h = (wh - lty - ((r - 1) * m->gappx)) / r;
				resize(c, wx, wy + lty, tw - (2*c->bw), h - (2*c->bw), False);
				lty += HEIGHT(c) + m->gappx;
			} else { 
				/* Put on right stack */
				r = (1 + n - i) / 2;	
				h = (wh - rty - ((r - 1) * m->gappx)) / r;
				resize(c, rtx, wy + rty, tw - (2*c->bw), h - (2*c->bw), False);
				rty += HEIGHT(c) + m->gappx;
...
{
	Client *c;
	int i, n, r, mw, mh, mx, my, mrx, sx, w,
	    ww, wh,wx, wy;
	...
	/* Applying edgepx to window area */
	ww = m->ww - (m->edgepx * 2);
	wh = m->wh - (m->edgepx * 2);
	wx = m->wx + m->edgepx;
	wy = m->wy + m->edgepx;
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			r = MIN(m->nmaster, n) - i;
			w = (mw - mrx - ((r - 1) * m->gappx)) / r;
			resize(c, mx + mrx, my, w - (2*c->bw), mh - (2*c->bw), False);
			mrx += WIDTH(c) + m->gappx;
		} else {
			r = n - i;
			w = (ww - sx - ((r - 1) * m->gappx)) / r;
			resize(c, wx + sx, wy, w - (2*c->bw), wh - (2*c->bw), False);
			sx += WIDTH(c) + m->gappx;
		}
	}
}
...
doubledeck(Monitor *m)
{
	...
	/* Geometry of window area with edge gaps */
	ww = m->ww - (m->edgepx * 2);
	wh = m->wh - (m->edgepx * 2);
	wx = m->wx + m->edgepx;
	wy = m->wy + m->edgepx;
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster)
			resize(c, wx, wy, mw - (2*c->bw), wh - (2*c->bw), False);
		else
			resize(c, dx, wy, dw - (2*c->bw), wh - (2*c->bw), False);
	}
...
{
	Client *c;
	unsigned int n, r, sx, my, sy, mw, sw, h,
		     ww, wh, wx, wy, i;

	/* Calculating number of til windows */
	for (n = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), n++);
	if (n == 0)
		return;
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			r = MIN(n, m->nmaster) - i;
			h = (wh - my - ((r -1) * m->gappx)) / r;
			resize(c, wx, my + wy, mw - (2*c->bw), h - (2*c->bw), False);
			my += HEIGHT(c) + m->gappx;
		} else {
			/* Put in stack */
			r = n - i;
			h = (wh - sy - ((r -1) * m->gappx)) / r;
			resize(c, sx, sy + wy, sw - (2*c->bw), h - (2*c->bw), False);
			sy += HEIGHT(c) + m->gappx;
		}
	}
}
...
{
	Client *c;
	unsigned int n, r, sy, mx, sx, mh, sh, w,
		     ww, wh, wx, wy, i;

	/* Calculating number of tiled windows */
	for (n = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), n++);
	if (n == 0)
		return;
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			/* Put in master */
			r = MIN(n, m->nmaster) - i;
			w = (ww - mx - ((r -1) * m->gappx)) / r;
			resize(c, wx + mx, wy, w - (2*c->bw), mh - (2*c->bw), False);
			mx += WIDTH(c) + m->gappx;
		} else {
			/* Put in stack */
			r = n - i;
			w = (ww - sx - ((r - 1) * m->gappx)) / r;
			resize(c, wx + sx, sy, w - (2*c->bw), sh - (2*c->bw), False);
			sx += WIDTH(c) + m->gappx;
		}
	}
}
